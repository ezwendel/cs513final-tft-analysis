...
# read in data
data <- read.csv("C://Users//elijah//Desktop//cs513final//finalDataALL//pca_data.csv", na.strings="?")
...
saveRDS(C50_class, "./PCA_all_C50.rds")
c50Model <- readRDS("./PCA_all_C50.rds")
...
saveRDS(CART_class, "./PCA_all_CART.rds")
cartModel <- readRDS("./PCA_all_CART.rds")
...
saveRDS(nBayes_all, "./PCA_all_nb.rds")
nbModel <- readRDS("./PCA_all_nb.rds")
...
saveRDS(predict_k215, "./PCA_all_knn.rds")
knnModel <- readRDS("./PCA_all_knn.rds")

# neural nets

# have to make dataset smaller for the neural net
...
library("neuralnet")
class(training$top4)
net_wisc_bc<- neuralnet( top4~. , data=training, hidden=3, threshold=0.01, linear.output = FALSE, rep=2)
...
nn_wrong<- (test$top4!=ann_cat[,2])
nn_error_rate<-sum(nn_wrong)/length(nn_wrong)
nn_error_rate

saveRDS(net_wisc_bc, "./PCA_nn_all.rds")

net_wisc_bc <- readRDS('./PCA_nn_all.rds')
